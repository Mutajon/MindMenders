...
class MovementBorderComponent extends PositionComponent
  ...
  MovementBorderComponent({
    this.baseColor = const Color(0xFF448AFF),
    int priority = 200,
  ...
  void _recalculatePath() {
    ...
    final edgeMapping = {0: 2, 1: 5, 2: 4, 3: 1, 4: 3, 5: 0};
    ...
    for (final tile in _tiles) {
      ...
      for (int i = 0; i < 6; i++) {
        ...
        final isNeighborInSet =
            neighborTile != null && _tiles.contains(neighborTile);
  ...
  void render(Canvas canvas) {
    ...
    final startOffset = Offset.lerp(
      const Offset(0, 0),
      const Offset(50, 50),
      t,
    )!;
    final endOffset = Offset.lerp(
    ...
    final color2 = HSVColor.fromColor(
      baseColor,
    ).withHue((HSVColor.fromColor(baseColor).hue + 20) % 360).toColor();
